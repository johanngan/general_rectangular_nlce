function psi_out = sum_sigma_y(psi_in, weights)
    % Applies the sum (weighted or unweighted) of the sigma_y Pauli
    ...
    if nargin < 2   % unweighted - saves a bit of computation
        psi_out = sum_local(psi_in, @sigma_y);
    else    % weighted
        psi_out = sum_local(psi_in, @sigma_y, weights);
    end
end